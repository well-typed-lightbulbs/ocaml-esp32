...
CAMLexport void caml_close_channel(struct channel *channel)
{
  ...
  if (channel->refcount > 0)
    return;
  if (caml_channel_mutex_free != NULL)
    (*caml_channel_mutex_free)(channel);
...
CAMLexport void caml_finalize_channel(value vchan)
{
  ...
  if ((chan->flags & CHANNEL_FLAG_MANAGED_BY_GC) == 0)
    return;
  if (--chan->refcount > 0)
    return;
  if (caml_channel_mutex_free != NULL)
    (*caml_channel_mutex_free)(chan);
...
CAMLprim value caml_ml_set_binary_mode(value vchannel, value mode)
{
  ...
  if ((channel->flags & CHANNEL_FLAG_FROM_SOCKET) != 0 && !Bool_val(mode))
  {
    errno = EINVAL;
    caml_sys_error(NO_ARG);
  }
