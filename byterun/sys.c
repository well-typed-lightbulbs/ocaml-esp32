...
CAMLprim value caml_sys_exit(value retcode_v)
{
  ...
  {
    /* cf caml_gc_counters */
    double minwords = caml_stat_minor_words + (double)(caml_young_end - caml_young_ptr);
...
#if defined(F_SETFD) && defined(FD_CLOEXEC) && !defined(_WIN32) && !defined(O_CLOEXEC)
  if (fd != -1)
...
double caml_sys_time_include_children_unboxed(value include_children)
{
  ...
  acc += ru.ru_utime.tv_sec + ru.ru_utime.tv_usec / 1e6 + ru.ru_stime.tv_sec + ru.ru_stime.tv_usec / 1e6;
