...
void caml_shrink_heap(char *chunk)
{
  ...
  */
  ...
  caml_gc_message(0x04, "Shrinking heap to %" ARCH_INTNAT_PRINTF_FORMAT "uk words\n",
...
{
  if (caml_gc_phase == Phase_mark || caml_gc_phase == Phase_clean || (caml_gc_phase == Phase_sweep && (addr)hp >= (addr)caml_gc_sweep_hp))
  {
    return Caml_black;
  }
  else
  {
    CAMLassert(caml_gc_phase == Phase_idle || (caml_gc_phase == Phase_sweep && (addr)hp < (addr)caml_gc_sweep_hp));
    return Caml_white;
  }
}
...
CAMLexport void caml_adjust_gc_speed(mlsize_t res, mlsize_t max)
{
  ...
  if (caml_extra_heap_resources > (double)caml_minor_heap_wsz / 2.0 / (double)caml_stat_heap_wsz)
  {
